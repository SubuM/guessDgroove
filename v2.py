...
def page_lobby():
        ...
        if st.button("📂  Load Playlist", use_container_width=True) and url.strip():
            with st.spinner("Fetching playlist..."):
                try:
                    ...
                    db = get_db()
                    try:
                        room_db = db.query(GameRoom).filter(GameRoom.id == st.session_state.code).first()
                        if not room_db:
                            st.error("Room not found in database")
                            return
                        room_db.playlist_url = url
                        room_db.playlist_source = source
                        room_db.total_rounds = len(tracks)
                        room_db.state = "lobby"
                        for i, t in enumerate(tracks):
                            db.add(Track(room_id=room_db.id, round_number=i + 1, title=t.get("title", ""),
    ...
    # Auto-refresh for non-hosts to pick up state changes
