"""
guessDgroove v4 - Song Prelude Extractor
Extract individual songs from a YouTube video and play a short intro prelude of each.
Run with: streamlit run v4.py
"""
...
if st.button("🔍  Analyze & Extract Preludes", type="primary", use_container_width=True) and url.strip():
    with st.status("Processing...", expanded=True) as status:
        ...
        status.update(label=f"✅ Done — {len(segments)} preludes extracted", state="complete")
            ...
            <div class="seg-card">
                <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
                    ...
                    <div>
                        ...
                        <div class="prelude-label">▶ prelude ({prelude_sec}s)</div>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
