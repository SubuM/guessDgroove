"""
guessDgroove v7 - Song Prelude Quiz
Play a prelude of each detected song and let the user guess the song name.
Run with: streamlit run v7.py
"""
...
def check_guess(guess: str, actual: str) -> bool:
    ng, na = _normalize(guess), _normalize(actual)
    if not ng or not na:
        return False
    if ng == na or na in ng or ng in na:
        return True
    return difflib.SequenceMatcher(None, ng, na).ratio() >= 0.8
...
url = st.text_input("YouTube URL", key="v7_url", placeholder="https://www.youtube.com/watch?v=...",
...
if results and results["url"] == url.strip():
    ...
    medley_path = Path(results["medley_path"])
    ...
    for seg, prelude_path in zip(segments, prelude_paths):
        ...
        guess_key = f"v7_guess_{idx}"
        checked_key = f"v7_checked_{idx}"
        ...
        with st.container():
            ...
            with col_btn:
                if st.button("Check", key=f"v7_btn_{idx}",
                            disabled=st.session_state[checked_key],
            ...
            if st.session_state[checked_key]:
                ...
                if title and guess.strip():
                    correct = check_guess(guess, title)
                    if correct:
