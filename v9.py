...
if "v9_stage" not in st.session_state:
    st.session_state["v9_stage"] = "upload"
if "v9_guess_min_ratio" not in st.session_state:
    st.session_state["v9_guess_min_ratio"] = 0.85
if "v9_quiz_idx" not in st.session_state:
    st.session_state["v9_quiz_idx"] = 0
...
if st.session_state["v9_stage"] == "upload":
    url = st.text_input("YouTube URL", key="v9_url",
    ...
    with st.expander("⚙️  Settings", expanded=False):
        ...
        guess_min_ratio = st.slider("Guess check strictness", 0.50, 1.00, 0.85, 0.05,
                                    help="Minimum similarity score needed for a guess to count as correct. "
                                         ...
                                         "Lower = more lenient.")
        st.session_state["v9_guess_min_ratio"] = guess_min_ratio
    ...
    if results and results["url"] == url.strip():
        st.success(f"✅ Analysis complete — **{len(results['segments'])} songs detected**. "
                   "Ready to play the quiz!")
        if st.button("🚀  START", type="primary", use_container_width=True):
            st.session_state["v9_stage"] = "quiz"
            st.rerun()
    ...
    guess_min_ratio = st.session_state["v9_guess_min_ratio"]

    st.markdown(f"### 🎧  Quiz — {video_title}")
    ...
    with col_top[1]:
        st.markdown(f"<p style='text-align:right;color:#9090b0'>Song "
                    f"{st.session_state['v9_quiz_idx'] + 1} of {len(segments)}</p>",
                    unsafe_allow_html=True)

    st.divider()

    quiz_idx = st.session_state["v9_quiz_idx"]
    if quiz_idx >= len(segments):
        ...
        for i, seg in enumerate(segments):
            idx = seg["index"]
            title = seg.get("title")
            guess = st.session_state.get(f"v9_guess_{idx}", "").strip()
            checked = st.session_state.get(f"v9_checked_{idx}", False)
            if not checked:
                status_icon, status_label = "⏭️", "Skipped"
            elif not title:
                status_icon, status_label = "➖", "Not graded"
            elif check_guess(guess, title, min_ratio=guess_min_ratio):
                status_icon, status_label = "✅", "Correct"
            else:
                status_icon, status_label = "❌", "Wrong"
            rows.append((i, idx, title, guess, status_icon, status_label, checked))

        score = sum(1 for r in rows if r[5] == "Correct")
        answered = sum(1 for r in rows if r[6])
        ...
        for i, idx, title, guess, icon, status_label, checked in rows:
            t = title if title else "Unnamed song"
    ...
    guess_key = f"v9_guess_{idx}"
    checked_key = f"v9_checked_{idx}"
    ...
    with st.container():
        ...
        with col_inp:
            st.text_input("Your guess", key=guess_key,
                         disabled=st.session_state[checked_key],
                         placeholder="Type song name...")
        with col_btn:
            if st.button("Check", key=f"v9_btn_{idx}",
                        disabled=st.session_state[checked_key],
        ...
        if st.session_state[checked_key]:
            ...
            if title and guess.strip():
                correct = check_guess(guess, title, min_ratio=guess_min_ratio)
                if correct:
                    st.success(f"✅ Correct! → **{title}**")
                else:
                    st.error(f"❌ Wrong! Actual: **{title}**")
            elif title:
                st.warning(f"Answer: **{title}**")
            else:
                st.info("No chapter name available for this segment")
    ...
    with nav_cols[0]:
        if quiz_idx > 0 and st.button("←  Previous", use_container_width=True):
            st.session_state["v9_quiz_idx"] = quiz_idx - 1
            st.rerun()
    with nav_cols[1]:
        if quiz_idx < len(segments) - 1:
            if st.button("Next →", key="v9_next", type="primary",
                         use_container_width=True):
                st.session_state["v9_quiz_idx"] = quiz_idx + 1
                st.rerun()
        else:
            if st.button("Finish ✅", key="v9_finish", type="primary",
                         use_container_width=True):
                st.session_state["v9_quiz_idx"] = len(segments)
                st.rerun()
